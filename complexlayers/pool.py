...
class _SpectralPooling(Layer):
    ...
    def call(self, inputs, mask=None):
        ...
        input_complex = tf.complex(input_real, input_image)
        t = tf.unstack(input_complex, axis=-1)       # 在最后一个维度分解矩阵，会降低一维
        spectral_input = [tf.expand_dims(self.fftFunc(k), -1) for k in t]
        spectral_input = tf.concat(spectral_input, -1)    # 将分解的做ttf，并重新组合，concat不增加新的维度，变成了一个channel
        ...
        output_image = tf.math.imag(output)
